...
import hub.sam.models.sdlplusconcretesyntaxextensions.ChannelEndPointBinding;
import hub.sam.models.sdlplusconcretesyntaxextensions.ConcreteSyntaxExtension;
import hub.sam.models.sdlplusconcretesyntaxextensions.Identifier;
...
public class SemanticAnalysis {
    ...
    public boolean doSemanticAnalysis(Repository repository, Extent meta, Extent inModel) {
        ...
        final ResolveConcreteSyntaxExtensions resolver = new ResolveConcreteSyntaxExtensions(repository, meta, inModel, errors);
        ...
        if (new AnalysisStep() {
            @Override
            void doStep() {
                resolver.createInterfaces();
                resolver.resolveChannelEndPointBindings();
            }
        }.execute(errors)) {
            return false;
        }
        ...
        for (Object cse : inModel.getObject()) {
            if (cse instanceof ConcreteSyntaxExtension) {
                System.out.println("#### reference left: " + cse.getClass().getName());
                Identifier ident;
                if (cse instanceof Identifier) {
                    ident = (Identifier)cse;
                } else if (cse instanceof ChannelEndPointBinding) {
                    ident = ((ChannelEndPointBinding)cse).getGateIdentifier();
                    System.out.println("ChannelEndPointBinding:");
                } else {
